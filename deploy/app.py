...
def get_predictions():
    ...
    last_update = os.path.getmtime(run_backend.db_name) * 1e9
...
#        predictions_formatted.append("<tr><th><a href=\"{link}\">{title}</a></th><th>{score}</th></tr>".format(title=e[1], link=e[0], score=e[2]))
    ...
    for n, e in enumerate(predictions):
        if n == 0:
            ...
            </div>""".format(embed=e[0].replace("https://www.youtube.com/watch?v=", "https://www.youtube.com/embed/"), score=e[2]))
        elif n !=0 and n <= 4:
...
def main_page():
    ...
    topo = codecs.open("topo.html", "r", "utf-8").read().format((time.time() - last_update) / 1e9, carrousel) #layout baseado no git  https://github.com/hugotrigueiro/Youtube-Video-Recommender/ - site https://ytvideo-recommender-by-hugo.herokuapp.com/
    final = codecs.open("final.html", "r", "utf-8").read()
    result = topo + """
            <table class="table-videos">{}</table>""".format(preds) + final
